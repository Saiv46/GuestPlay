...
window.GuestPlay = {
	start: (placeId, userId, gender) => {
		if(~blacklist.indexOf(placeId) &&
			window.confirm && !window.confirm(chrome.i18n.getMessage("blacklisted"))) return;
	...
	play: async (forceId) => Promise.all([forceId || GuestPlay.currentTab, GuestPlay.id, GuestPlay.gender]).then(data => GuestPlay.start(...data)),
	
	get currentTab(){
		return new Promise((res, rej) => chrome.tabs.query({active: true, currentWindow: true, url: "https://*.roblox.com/games/*/*"}, tabs => {
	...
	set currentTab(analytics){
		return chrome.tabs.create({url: analytics ? "https://web.roblox.com/games/?SortFilter=default&TimeFilter=0&utm_source=guestplay&utm_medium=organic&utm_campaign=play&utm_content=plugin" : "https://web.roblox.com/games/"})
	},
	
	get gender(){
		return new Promise((res, rej) => chrome.storage.local.get(['gender'], data => {data.gender !== undefined ? res(data.gender) : rej()}));
	},
	set gender(value){
		return new Promise((res, rej) => chrome.storage.local.set({gender: value}, () => {chrome.runtime.lastError ? rej(chrome.runtime.lastError) : res(value)}))
	...
	get id(){
		return new Promise((res, rej) => chrome.storage.local.get(['id'], data => {data.id !== undefined ? res(data.id) : rej()}));
	},
	set id(value){
		return new Promise((res, rej) => chrome.storage.local.set({id: value}, () => {chrome.runtime.lastError ? rej(chrome.runtime.lastError) : res(value)}))
			.catch(err => {
				console.error(err);
				alert(chrome.i18n.getMessage("saveFailed") || "Failed to save your preferences, please reload this page or contact developers about this problem.");
			})
	...
	init() {
		...
		document.getElementById("mailto").onclick = () => chrome.tabs.create({url: `mailto:45tanaa@gmail.com?subject=${encodeURIComponent('GuestPlay ['+ chrome.i18n.getUILanguage().toUpperCase() +']')}&body=${encodeURIComponent(chrome.i18n.getMessage("contactUsMessage"))}`});
		...
		document.querySelectorAll("#gender li").forEach(a => {
			...
			a.onclick = () => {GuestPlay.gender = genderId};
		});
		document.getElementById("play").title = chrome.i18n.getMessage("play") || "Play as Guest";
		GuestPlay.id.catch(() => {GuestPlay.id = Math.floor(Math.random() * 9999999999)});
		GuestPlay.gender.then(
			gender => document.body.setAttribute("gender", gender),
			() => {GuestPlay.gender = 0}
		);
	}
};

(f => {(document.attachEvent ? document.readyState === "complete" : document.readyState !== "loading") ? f() : document.addEventListener('DOMContentLoaded', f)})(GuestPlay.init);